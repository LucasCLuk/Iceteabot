...
class SuggestionPaginator(Pages):
    def __init__(self, ctx, entries, *, per_page=1):
        super().__init__(ctx, entries=entries, per_page=per_page)
        self.total = len(entries)
        self.reaction_emojis.append(("\U0001f44c", self.mark_as_finished))

    async def show_page(self, page, *, first=False):
        self.current_page = page
        entry = self.get_page(page)[0]

        self.embed.clear_fields()
        self.embed.description = f"Iceteabot Suggestions. Page {page}/{self.maximum_pages}"
        self.embed.set_author(name=self.author, icon_url=self.author.avatar_url)

        self.embed.add_field(name="id", value=entry.id, inline=False)
        self.embed.add_field(name="status", value=entry.status)
        self.embed.add_field(name="upvotes", value=entry.up_votes)
        self.embed.add_field(name="suggestion", value=entry.suggestion, inline=False)
    ...
    async def mark_as_finished(self):
        suggestion = self.get_page(self.current_page)[0]
        suggestion.status = "\U0001f44c"
        await suggestion.save()


class CommandStatsPaginator(Pages):
    ...
    async def show_page(self, page, *, first=False):
        ...
        for command, amount in entries:
            self.embed.add_field(name=command, value=amount)
...
class OsuHistoryPaginator(Pages):
    def __init__(self, ctx, entries, *, per_page=1, user_data, game_mode):
        super().__init__(ctx, entries=entries, per_page=per_page)
        self.total = len(entries)
    ...
    async def show_page(self, page, *, first=False):
        self.current_page = page
        entry = self.get_page(page)[0]
        play_date = entry.date.replace(tzinfo=pytz.timezone("Australia/Perth"))
        beat_map_data = await self.bot.osu_client.get_beatmaps(mode=self.game_mode, beatmap_id=entry.beatmap_id)
        self.embed.clear_fields()
        self.embed.description = f"{self.user_data.username}, {self.game_mode} Recent plays"
        self.embed.url = "https://new.ppy.sh/u/{0}".format(self.user_data.user_id)
...
class OsuBestPaginator(Pages):
    def __init__(self, ctx, entries, *, per_page=1, user_data, game_mode, osu_client):
        super().__init__(ctx, entries=entries, per_page=per_page)
        self.total = len(entries)
        self.user_data = user_data
        self.game_mode = game_mode
    ...
    async def show_page(self, page, *, first=False):
        self.current_page = page
        entry = self.get_page(page)[0]
        beat_map_data = await self.osu.get_beatmaps(mode=self.game_mode, beatmap_id=entry.beatmap_id)
        self.embed.clear_fields()
        self.embed.description = f"{self.user_data.username}, {self.game_mode} Best plays"
        self.embed.url = "https://new.ppy.sh/u/{0}".format(self.user_data.user_id)
        self.embed.title = f"{beat_map_data[0].title}"
        self.embed.set_author(name=self.author, icon_url=self.author.avatar_url)
        self.embed.colour = 0x58D68D
...
class TaskPaginator(Pages):
    ...
    async def show_page(self, page, *, first=False):
        ...
        entry = self.get_page(page)[0]
        self.embed.clear_fields()
        self.embed.description = f"{self.author} Tasks"
        self.embed.set_author(name=self.author, icon_url=self.author.avatar_url)
        self.embed.colour = 0x58D68D
        self.embed.add_field(name=f"Task #{entry.number}", value=entry.content)
        self.embed.add_field(name="Finished", value="\U00002705" if entry.finished else "\U0000274c")
        if not self.paginating:
            return await self.channel.send(embed=self.embed)
...
class TagPaginator(Pages):
    async def show_page(self, page, *, first=False):
        self.current_page = page
        entry = self.get_page(page)
        self.embed.clear_fields()
        self.embed.set_author(name=self.author, icon_url=self.author.avatar_url)
        self.embed.colour = self.author.top_role.color
        self.embed.add_field(name="\u200b",
                             value="\n".join([f"{self.entries.index(tag) + 1}. {tag}" for tag in entry]))
        if not self.paginating:
            return await self.channel.send(embed=self.embed)
        if not first:
            return await self.message.edit(embed=self.embed)
